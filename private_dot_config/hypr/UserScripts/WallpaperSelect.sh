...
DURATION=2
BEZIER=".43,1.19,1,.4"
SWWW_PARAMS="--resize crop --transition-fps $FPS --transition-type $TYPE --transition-duration $DURATION --transition-bezier $BEZIER"
...
apply_video_wallpaper() {
  ...
  mpvpaper '*' -o "no-audio --loop --keepaspect --panscan=1.0 --hwdec=auto --scale=bilinear --cache=yes --demuxer-max-bytes=150M --demuxer-max-back-bytes=75M --no-resume-playback" "$video_path" &
}
